...
| `GET /api/update/channels` | 无 | 只读静态元数据（镜像 URL、发布版本口径）；**不**访问 Velopack feed |
...
| `WS /ws/logs`、`/ws/status` | 首次消息认证或 Query `ws_token` | 需与 session token 一致（向后兼容） |

**假设**：信任本机用户；本机其他进程或恶意软件若可访问 `127.0.0.1:18765`，可能读取 session token 或连接 WebSocket。请勿将控制台端口暴露到局域网或公网，勿在不可信的多用户环境中运行。

## 社区网站 CORS 配置

W-SECURITY-003: 社区注册守卫 Edge Function 的 CORS Origin 已从通配符 `*` 改为环境变量 `COMMUNITY_CORS_ORIGIN` 配置的域名（逗号分隔支持多域名）。未配置时使用默认值 `https://community.danmuai.com`。
