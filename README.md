...
在控制台的模型设置中填写模型平台、API Key、端点和模型名。OpenAI 平台的内置模型目录当前仅包含 GPT-5.6 Sol、GPT-5.6 Terra 和 GPT-5.6 Luna；其它 OpenAI 模型仍可通过自定义模型功能添加。项目已适配的平台与模型能力以当前版本的配置界面为准；自定义模型请确认其接口兼容项目支持的请求格式。未单独绑定模型的人格会使用模型列表中的第一个模型；项目不再提供全局默认模型选择。
