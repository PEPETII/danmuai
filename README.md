...
DanmuAI 是一个 Windows / macOS 桌面弹幕工具：截取**所选显示器全屏**，调用视觉模型生成 5 条弹幕，并以 Qt 透明置顶浮层滚动展示。默认通过 **温馨 Web 控制台**（pywebview 桌面壳）配置与启停；Qt 仅负责弹幕 Overlay 与系统托盘。
...
<img width="1895" height="1205" alt="屏幕截图 2026-05-25 210707" src="https://github.com/user-attachments/assets/76cbded4-c940-4d5b-a577-e3cd57cc5ef3" />
<img width="1348" height="2397" alt="qrcode_1779738450536" src="https://github.com/user-attachments/assets/6c97b06c-867a-4f2b-80db-904e9ad83212" />


**项目定位**：为直播主播提供轻量、隐私友好的 AI 弹幕助手。截图在内存中压缩后发送模型，默认不落盘；配置与密钥存于本机用户数据目录（Windows `%APPDATA%/DanmuAI/`，macOS `~/Library/Application Support/DanmuAI/`）。
...
| **pywebview** | 桌面壳（Windows WebView2 / macOS Cocoa WebKit） |
...
| **keyboard** | Windows 全局快捷键（macOS 默认用托盘 / Web 控制） |
| **python-Levenshtein** | 弹幕去重相似度计算 |

## 功能特性

- 弹幕显示模式：**普通模式（默认）**（可配置每 x 秒识别、每次 y 条弹幕）或实时模式（1 秒截图 + 节奏预触发）；实时模式下画面/泛用条数各 2–7 条可调
- 主线程截图，线程池压缩和 AI 请求，避免 UI 阻塞
- 过期 `screenshot_id` / 场景代际回复自动丢弃
- 连续失败退避、超时控制、日志脱敏
...
- **Python** ≥ 3.12
- **平台**：Windows 10/11（WebView2 用于 pywebview 壳）或 macOS（Cocoa/WebKit）
...
```bash
python main.py                         # pywebview 桌面壳 + 托盘 + Qt 弹幕 Overlay
...
桌面壳为 **pywebview Cocoa/WebKit**。构建、权限与排错见 **[docs/PACKAGING_MACOS.md](docs/PACKAGING_MACOS.md)**。
...
产物：`dist/DanmuAI.app`。运行诊断：`~/Library/Application Support/DanmuAI/startup.log`。

| 环境变量 | 说明 |
|----------|------|
| `DANMU_WEB_LAUNCH=browser` | 强制系统浏览器（等同 `--web-browser`） |
| `DANMU_WEB_LAUNCH=webview` | 强制 pywebview 桌面壳 |
...
2. 填写 `API Endpoint`、`API Key`、`Model`；可选用服务商预设快速填入。
3. 在「节奏与截图策略」「图像压缩预览」中调整参数；多屏时在「显示器」下拉选择目标屏。
4. 点击 **保存配置**，再在 **温馨控制台** 点击 **生成弹幕**。

人格与提示词在侧栏 **人格工坊**；自定义模型在设置页「自定义模型」卡片中管理。

项目提供 [`.env.example`](.env.example) 作参考。**注意**：桌面应用默认通过 Web/设置写入本机配置库（Windows `%APPDATA%/DanmuAI/config.db`，macOS `~/Library/Application Support/DanmuAI/config.db`），不会自动加载 `.env`。
...
- API Key 存储在本机配置库（Windows `%APPDATA%/DanmuAI/config.db`，macOS `~/Library/Application Support/DanmuAI/config.db`），优先 Fernet 加密；缺少 `cryptography` 时退化为 base64 并警告。
...
- 请求前有下限保护（普通 ≥512，thinking ≥1024）。

### Web 控制台打不开？

- 确认 `127.0.0.1:18765` 未被占用；可试 `--web-browser`。
- Windows 需 WebView2 运行时；macOS 需 Cocoa/WebKit 依赖（见 `requirements.txt`）。
...
| [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) | 架构与 Web 控制台 |
...
└─ requirements.txt
```
<img width="1210" height="1210" alt="mm_reward_qrcode_1779738306814" src="https://github.com/user-attachments/assets/fd778bd6-02e9-4280-a210-f7951f81092a" />
