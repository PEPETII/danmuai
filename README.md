...
详见 [docs/WEB_CONSOLE.md](docs/WEB_CONSOLE.md)、[docs/ROADMAP.md](docs/ROADMAP.md)、[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)。
...
- **当前安装包未代码签名**，Windows 可能显示 SmartScreen「未知发布者」——见 [docs/operations/WINDOWS_CODE_SIGNING.md](docs/operations/WINDOWS_CODE_SIGNING.md)。选择「更多信息 → 仍要运行」即可。
...
| `DANMU_SUPABASE_URL` | 可选；后端读取 `app_updates`（覆盖 `web/static/supabase-config.js` 中的 `url`） |
| `DANMU_SUPABASE_ANON_KEY` | 可选；与上项配对使用 |

**Supabase（公告 / 反馈 / 更新元数据）**：复制 `web/static/supabase-config.example.js` → `web/static/supabase-config.js` 并填写项目 URL 与 anon key；打包时随 `web/static` 分发。发版后须在 Supabase `app_updates` 表维护 `latest_version` / `release_url`，详见 [`supabase/README.md`](supabase/README.md) 与 [`docs/operations/RELEASE_CHECKLIST.md`](docs/operations/RELEASE_CHECKLIST.md)。

更多运行时细节见 [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)、[docs/CONTRIBUTING_ARCHITECTURE.md](docs/CONTRIBUTING_ARCHITECTURE.md)。
...
| [docs/MAIN_PIPELINE.md](docs/MAIN_PIPELINE.md) | 主链路（截图 → AI → 弹幕） |
