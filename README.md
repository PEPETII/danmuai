...
7. 🧩 **可选扩展能力**：包含麦克风输入、虚拟主播 Live2D 模型导入、烂梗弹幕和知识库等模块，按当前版本与配置启用；虚拟主播页可从已导入模型的下拉菜单切换，未导入时显示“无”；Live2D 模型启动后显示在独立的 Windows 桌面窗口，控制台只负责控制。
...
在控制台的模型设置中填写模型平台、API Key、端点和模型名。内置国内平台目录包含基元律动（Token Rhythm）的 `qwen3.7-flash`，该模型当前标记为“测试中”；OpenAI 平台的内置模型目录当前仅包含 GPT-5.6 Sol、GPT-5.6 Terra 和 GPT-5.6 Luna；其它 OpenAI 模型仍可通过自定义模型功能添加。项目已适配的平台与模型能力以当前版本的配置界面为准；自定义模型请确认其接口兼容项目支持的请求格式。未单独绑定模型的人格会使用模型列表中的第一个模型；项目不再提供全局默认模型选择。
...
<details>
<summary>启用麦克风、虚拟主播、烂梗弹幕和知识库</summary>

这些能力属于可选模块。虚拟主播位于侧边栏【虚拟主播】，是否需要额外的模型或音频服务取决于当前版本和本地配置；遇到问题时请附上脱敏后的操作步骤与日志摘要。

启用虚拟主播后，侧边栏教程区域提供“虚拟主播日志”页签，用于查看当前进程最近生成的虚拟主播说话内容。
