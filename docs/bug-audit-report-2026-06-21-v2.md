...
> **历史审计快照**：结论仅对应 commit `32ee87a`、版本 `0.3.4` 与 2026-06-21 的工作树。该版本不是现行入口；当前问题状态见 [.local-ai/workorders/已知问题与后续事项.md](../.local-ai/workorders/已知问题与后续事项.md)。
>
> **历史路径说明**：`app/config_store.py`、`app/danmu_engine.py` 的旧单文件目标已删除或拆分，无法在不改变历史含义的前提下一一重定向，故保留旧目标并明确标注。
...
- 严重等级：**P2**
- 影响功能：弹幕渲染性能
- 证据文件：[overlay.py](../app/overlay.py)
- 证据代码：
  ```python
  # L69-74
...
- 严重等级：**P2**
- 影响功能：自动更新版本比较
- 证据文件：[version_compare.py](../app/version_compare.py)
- 证据代码：
  ```python
  # L38-43
...
- 证据文件：[reply_parser.py](../app/reply_parser.py) L130-173
...
- 证据文件：[web_console.py](../app/web_console.py) L186-231
...
- **httpx 超时**（[ai_client.py](../app/ai_client.py) L100）：`httpx.Timeout(30.0, connect=5.0)`，30 秒总超时合理。但流式响应中无总超时限制（`stream_openai` 仅检查 `_request_wall_clock_exceeded`），若模型持续返回 SSE 事件但内容为空，可能长时间不超时。
- **重试策略**：最多 2 次重试（L252, L460），超时重试、异常重试（重建 httpx 客户端），HTTP 状态错误不重试。设计合理。

### 5.3 Overlay 渲染

- **QPainterPath.addText 性能**（[overlay.py](../app/overlay.py) L96-106）：对 CJK/emoji 已走 fast 路径（`drawText` 描边），性能可接受。但 `_FAST_DANMU_RENDER_MIN_LEN = 36` 对纯 ASCII 短文本仍走慢路径，若弹幕内容为纯英文短句（如 "lol"），仍可能触发 `QPainterPath.addText`。实际影响取决于弹幕频率。
...
- **20000 条上限**（[danmu_pool.py](../app/danmu_pool.py) L17）：`CUSTOM_DANMU_POOL_MAX = 20000`。`custom_danmu_random_sample_for_store` 使用 `ORDER BY RANDOM() LIMIT ?`，对 20000 条数据性能可接受（SQLite RANDOM() 不需要全表排序）。
...
- **DanmuAI.spec**（[DanmuAI.spec](../DanmuAI.spec)）：hiddenimports 列表较完整，覆盖了 `app.*` 的所有子包。但以下新增模块可能遗漏：
...
- **velopack_runtime.py**（[velopack_runtime.py](../app/velopack_runtime.py)）：启动钩子在 `QApplication` 之前执行，异常处理合理（L32-33 捕获所有异常并日志记录，不阻塞启动）。
- **update_service.py**（[update_service.py](../app/update_service.py)）：`_manager()` 缓存 `UpdateManager` 实例，线程安全通过 `_lock` 保护。`download_updates` 中的线程管理合理。
...
- **凭据来源**（[supabase_config.py](../app/supabase_config.py)）：环境变量优先，回退到 `web/static/supabase-config.js`。打包时已排除此文件（DanmuAI.spec L69）。
- **anon_key 安全性**：Supabase anon key 设计上可公开，安全性依赖 RLS 策略。需确认 Supabase 项目中所有表都配置了正确的 RLS。

### 7.3 Web API 认证

- **Bearer token**（[web_console.py](../app/web_console.py) L402）：启动时生成 `secrets.token_urlsafe(24)`，仅绑定 `127.0.0.1`。设计合理。
