...
> **历史审计快照**：结论仅对应 commit `035acf9`、版本 `0.3.4` 与 2026-06-21 的工作树。现行状态见 [.local-ai/workorders/已知问题与后续事项.md](../.local-ai/workorders/已知问题与后续事项.md)，不得把本报告中的“建议修复”直接当作当前工单。
>
> **历史路径说明**：`app/config_store.py`、`app/danmu_engine.py` 的旧单文件目标已不再保留，旧行号无法唯一映射到现行子包；相关链接保留为历史证据，不伪造新目标。
...
- 证据文件：[main_lifecycle_mixin.py](../app/main_lifecycle_mixin.py)
- 证据代码：
  ```python
...
- 严重等级：**P2**
- 影响功能：悬浮窗弹幕渲染性能
- 证据文件：[floating_panel_overlay.py](../app/floating_panel_overlay.py)
- 证据代码：
  ```python
...
- 严重等级：**P2**
- 影响功能：悬浮窗弹幕去重准确性
- 证据文件：[floating_panel_engine.py](../app/floating_panel_engine.py)
- 证据代码：
  ```python
...
- 严重等级：**P2**
- 影响功能：AI 回复解析完整性
- 证据文件：[reply_parser.py](../app/reply_parser.py)
- 证据代码：
  ```python
  # line 100-127：
  if "}{" in raw:
...
- 证据文件：[doubao_responses_stream.py](../app/doubao_responses_stream.py)
- 证据代码：
  ```python
  # 第 116-123 行
...
- 证据文件：[ai_client_requests.py](../app/ai_client_requests.py)
- 证据代码：
  ```python
  # 第 240-241 行
  if temperature:
...
- 证据文件：[danmu_tts_playback.py](../app/danmu_tts_playback.py)
- 证据代码：
  ```python
  # 第 69-96 行
...
- 证据文件：[mic_capture.py](../app/mic_capture.py)
- 证据代码：
  ```python
  # 第 293-308 行
...
- 严重等级：**P2**
- 影响功能：读弹幕试听功能
- 证据文件：[danmu_read_service.py](../app/danmu_read_service.py)
- 证据代码：
  ```python
  # 第 273-321 行
...
- 严重等级：**P2**
- 影响功能：宠物素材包验证
- 证据文件：[pet_assets.py](../app/pet/pet_assets.py)
- 证据代码：
  ```python
  # 第 192 行
...
- 严重等级：**P2**
- 影响功能：桌宠弹幕模式切换
- 证据文件：[pet_barrage.py](../app/pet/pet_barrage.py)
- 证据代码：
  ```python
  # 第 127-132 行
  def show(self) -> None:
...
- 严重等级：**P3**
- 影响功能：宠物位置持久化
- 证据文件：[pet_window.py](../app/pet/pet_window.py)
- 证据代码：
  ```python
  # 第 866-877 行
...
- 证据文件：[danmu_pool.py](../app/danmu_pool.py)
- 证据代码：
  ```python
...
- 严重等级：**P1**
- 影响功能：烂梗公式化远程采集
- 证据文件：[client.py](../app/meme_barrage/client.py)
- 证据代码：
  ```python
  # 第 59 行
...
- 严重等级：**P2**
- 影响功能：烂梗标签列表展示
- 证据文件：[meme_barrage.py](../app/web_api/meme_barrage.py)
- 证据代码：
  ```python
  # 第 27 行
...
- 证据文件：[main_meme_mixin.py](../app/main_meme_mixin.py)
- 证据代码：
  ```python
  # 第 262-272 行
...
- 证据文件：[upload_github_release.ps1](../scripts/upload_github_release.ps1)
- 证据代码：
  ```powershell
  # 第 58 行
  if (-not $NotesFile) {
...
- 严重等级：**P1**
- 影响功能：Velopack 安装版启动
- 证据文件：[velopack_runtime.py](../app/velopack_runtime.py)
- 证据代码：
  ```python
  # 第 26-34 行
  try:
...
- 证据文件：[DanmuAI.spec](../DanmuAI.spec) + [supabase_config.py](../app/supabase_config.py)
- 证据代码：
  ```python
...
- 证据文件：[update_service.py](../app/update_service.py)
- 证据代码：
  ```python
  # 第 66-69 行
  def _manager():
...
- 严重等级：**P2**
- 影响功能：卸载时用户数据删除
- 证据文件：[uninstall_service.py](../app/uninstall_service.py)
- 证据代码：
  ```python
  # 第 81-88 行
...
- 证据文件：[001_announcements_feedback.sql](../supabase/migrations/001_announcements_feedback.sql), [supabase-client.js](../web/static/supabase-client.js)
- 证据代码：
  ```sql
...
- 严重等级：**P2**
- 影响功能：公告列表获取
- 证据文件：[supabase-client.js](../web/static/supabase-client.js)
- 证据代码：
  ```javascript
...
- 严重等级：**P2**
- 影响功能：直播弹幕层 SSE 推送
- 证据文件：[live_overlay.py](../app/web_api/live_overlay.py)
- 证据代码：
  ```python
...
- 严重等级：**P3**
- 影响功能：图像压缩预览
- 证据文件：[preview_compress.py](../app/web_api/preview_compress.py)
- 证据代码：
  ```python
...
- 严重等级：**P3**
- 影响功能：官网部署安全
- 证据文件：[project.json](../website/.vercel/project.json)
- 证据代码：
  ```json
  {
...
- 严重等级：**P3**
- 影响功能：测试基础设施兼容性
- 证据文件：[conftest.py](../tests/conftest.py)
- 证据代码：
  ```python
...
- 证据文件：[single_instance.py](../app/single_instance.py)
...
- 证据文件：[win32_overlay_zorder.py](../app/win32_overlay_zorder.py)
...
- 证据文件：[tts_providers.py](../app/tts_providers.py)
...
- 证据文件：[DanmuAI.spec](../DanmuAI.spec)
...
- 证据文件：[001_announcements_feedback.sql](../supabase/migrations/001_announcements_feedback.sql)
...
- 证据文件：[web_console_runtime.py](../app/web_console_runtime.py)
...
- 证据文件：[run_acceptance_gates.py](../scripts/run_acceptance_gates.py)
