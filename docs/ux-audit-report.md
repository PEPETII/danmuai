...
> **历史静态审计快照（2026-06-19）**：问题数量、组件路径和 WCAG 判断基于当时前端源码，未包含 2026-07 后续 UI 变更，也不等于已在真实浏览器/设备复现。`app/danmu_engine.py` 已拆为 `app/danmu_engine/` 子包；执行任何建议前须重新检查构建产物 `web/static/index.html` 与真实运行界面。
>
> 审计日期：2026-06-19
