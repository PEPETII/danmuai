...
> **历史审计快照**：结论仅对应 commit `1ac0f4a` 与 2026-06-19 的工作树，不是当前缺陷清单。现行状态见 [.local-ai/workorders/已知问题与后续事项.md](../.local-ai/workorders/已知问题与后续事项.md)；执行建议前须按当前源码复核。
>
> **历史路径说明**：仍指向 `app/config_store.py` 的链接对应当时的单文件实现；该目标已不再保留，无法可靠映射旧行号。当前代码为 `app/config_store/` 子包，但本文不据此改写历史证据。
...
- 证据文件：[.gitignore](../.gitignore)、[supabase-config.example.js](../web/static/supabase-config.example.js)
- 证据代码：
...
- 证据文件：[publish_windows_release.ps1](../scripts/publish_windows_release.ps1)
- 证据代码：
  ```powershell
...
- 证据文件：[webview_shell.py](../app/webview_shell.py)
...
- 证据文件：[mic_utterance.py](../app/mic_utterance.py)
