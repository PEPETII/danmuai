...
main.py::screenshot_timer.timeout
  ...
  -> main.py::_on_normal_capture_tick()
       ...
       -> main.py::_schedule_capture()
            -> app/snipper.py::build_capture_plan()        [main thread]
            -> capture_worker_pool().start(CaptureRunnable)
                 -> app/snipper.py::execute_capture()    [capture worker, grabWindow]
        ...
        -> main.py::_on_ai_reply()
           -> app/reply_parser.py::parse_ai_reply_payload()
...
|--------|-------------------|---------|------|
| `screenshot_timer` | `normal_recognition_interval_ms` | `_on_screenshot_timer` | Normal-mode capture schedule + API trigger on completion |
...
| `capture_worker_pool` | on demand | `CaptureRunnable.run` | W-PERF-HIGH-001: screen grab off main thread (max 1) |
| `ai_worker_pool` | on demand | `AiRunnable.run` / mic visual HTTP | S-014: isolated from global pool (max 2) |
| `QThreadPool` (global) | on demand | `_MicProbeRunnable.run` / `MemeFetchRunnable.run` / `MemeAiSelectRunnable.run` | Meme barrage remote fetch / AI select / mic probe |
...
| Capture tick | `_on_normal_capture_tick()` | `_schedule_capture`, `_on_capture_completed` → `_trigger_api_call` | skips if visual in-flight |
| Capture plan | `_schedule_capture()` | `build_capture_plan`, `CaptureRunnable` | sets `_capture_in_flight` |
| Capture worker | `CaptureRunnable.run()` | `execute_capture` | grab on capture pool |
| Capture apply | `_apply_capture_result()` | `_latest_screenshot*` | `_latest_screenshot_id++` only on valid frame |
...
- `screenshot_id`: set in `_apply_capture_result()` on valid frames only; carried on requests and queue items.
- `_capture_in_flight`: true while `CaptureRunnable` is queued/running; cleared in `_on_capture_completed`.
...
| `CaptureRunnable` | `app/worker_pools.capture_worker_pool()` (max 1) | W-PERF-HIGH-001: `grabWindow` off main thread |
...
Shutdown note: `DanmuApp.quit()` waits `capture_worker_pool().waitForDone(2000)` and `QThreadPool.globalInstance().waitForDone(2000)` before `ai_worker.close()`, so in-flight capture/AI workers do not touch closed clients during teardown.
