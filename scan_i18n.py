...
BASE = r'e:\test\danmu'
REPORT_PATH = os.path.join(BASE, 'i18n_scan_report.txt')
...
def main():
    ...
    ] + [os.path.join(BASE, 'web', 'static', 'partials', f) for f in os.listdir(os.path.join(BASE, 'web', 'static', 'partials')) if f.endswith('.html')]
    for path in html_files:
        ...
        for item in scan_html_file(path):
            ...
            lines.append(f'{path_out}:{line} | [{cat}] {snippet[:120]!r} | wrapped_attr={has_i18n} in_locale={in_dict} tag={tag}')
    ...
    for path in [app_js] + [os.path.join(modules_dir, f) for f in os.listdir(modules_dir) if f.endswith('.js')]:
