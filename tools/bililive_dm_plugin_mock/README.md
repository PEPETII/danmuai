# W-BILILIVE-DM-PLUGIN-MOCK-001 + BRIDGE-002/003 + PUSH-004 + FORMULA-008 — bililive_dm 本地显示桥接
...
> - FORMULA-008：公式化补位和烂梗弹幕复用同一 push 协议进入插件本地显示。
>
> **不**做：重写 DanmuAI 截图 / 视觉 AI / mic / pet / TTS 调度、向 B 站直播间自动发弹幕、复杂重试。双向本机 HTTP 已使用共享密钥校验；当前只在最终显示旁路接收 AI 与公式化文本，push 失败不会改变原主链路结果。
>
...
| 认证头 | `X-DanmuAI-Plugin-Secret`（读取 `%APPDATA%/DanmuAI/bililive_dm_plugin.secret`） |
...
**响应载荷**（BRIDGE-003 调用当前配置的真实模型，文本不固定）：
...
> **不**调用 DanmuAI 视觉主链路：截图 / 弹幕库 / mic / pet / TTS 都**不**接入；本路由由 `app/web_api/bililive_dm_bridge.py` 注册并委托 `app/application/bililive_dm_bridge_service.py` 做旁路真实 AI 生成。未配置模型时返回 `model_not_configured`（或具体凭据错误），不是固定测试文本。
...
| Content-Type | `application/json; charset=utf-8` |
| 认证头 | `X-DanmuAI-Plugin-Secret`（DanmuAI 与插件读取同一本机密钥） |
| 超时 | 3s（DanmuAI 侧 `httpx` 默认） |
| 实现 | `tools/bililive_dm_plugin_mock/src/DanmuAiPushListener.cs` |
| DanmuAI 契约 | `app/application/bililive_dm_contracts.py` |
...
FORMULA-008 复用完全相同的 HTTP 载荷。它不把公式化文本伪装成 AI 回复，也不进入 `_enqueue_reply_batch()`；当前只覆盖 `pool_topup`、`pool_duplicate_topup`、`meme_barrage` 三类最终显示来源。
...
| FORMULA-008 公式化旁路 | `pool_topup` / `pool_duplicate_topup` / `meme_barrage` | DanmuAI → 插件 | 18766 |
| 直播间发言 | — | **未实现** | — |
...
| # | 操作 | 预期结果 |
|---|------|----------|
| 1 | 启动 `bililive_dm`，进入「插件」页 | 看到 `DanmuAI Mock Plugin` 0.3.0-push，状态为「未启用」 |
...
### 5.2 bridge-002/003 本地旁路桥接（需先启动 DanmuAI）

> 前置：DanmuAI 已启动并监听 `127.0.0.1:18765`（`python main.py`），且已配置可用模型。可先在 PowerShell 自检；命令只把密钥留在进程变量中，不要输出或粘贴该值：
>
> ```powershell
> $pluginSecret = (Get-Content -LiteralPath "$env:APPDATA/DanmuAI/bililive_dm_plugin.secret" -Raw).Trim()
> $body = @{ room_id = 1; user_name = "alice"; user_id = "u1"; text = "hello" } | ConvertTo-Json
> Invoke-RestMethod -Method Post `
>   -Uri "http://127.0.0.1:18765/api/plugin/bililive-dm/reply" `
>   -Headers @{ "X-DanmuAI-Plugin-Secret" = $pluginSecret } `
>   -ContentType "application/json; charset=utf-8" `
>   -Body $body
> # 预期：ok=true，items 为 1-3 条当前模型生成文本；文本内容不固定。
> ```

| # | 操作 | 预期结果 |
|---|------|----------|
| 1 | 在任意直播间发一条评论（如 `bridge test hello`） | 弹幕姬日志出现收到评论与 bridge 成功记录；本地弹幕层出现 1-3 条真实模型生成文本，内容不固定 |
...
### 5.4 formula-008 公式化文本推送

| # | 操作 | 预期结果 |
|---|------|----------|
| 1 | 开启弹幕姬模式，并确保插件 listener 已启动 | 日志出现 `push listener started` |
| 2 | 触发 `pool_topup` 或 `pool_duplicate_topup` 补位 | DanmuAI 原显示链路不变；插件侧同步出现对应公式化文本 |
| 3 | 开启并触发一批 `meme_barrage` | 插件侧同步出现烂梗文本，且文本未进入 `_enqueue_reply_batch()` |
| 4 | 关闭插件后再次触发上述来源 | DanmuAI 只记录 push 失败，原 Overlay / 浮动面板 / 桌宠显示不受影响 |

---

## 6. 明确边界（PoC 不做的事）

- **不**重写 DanmuAI 主链路调度（截图 tick / reply_queue 内部）：AI 文本在 `_enqueue_reply_batch` 末尾旁路 POST；公式化文本在最终显示广播处旁路 POST，均不改主链时序。
- **不**实现「向 B 站直播间自动发弹幕」——`AddDM()` 写入的是弹幕姬本地层，仅供主播自己看见。
- **不**做远程身份系统、签名协议或复杂重试；双向本机 HTTP 仍强制校验 `X-DanmuAI-Plugin-Secret`。密钥默认位于 `%APPDATA%/DanmuAI/bililive_dm_plugin.secret`，不得提交、截图或贴入日志。
- **不**修改 `output/_bililive_dm_repo/`（只读参考）；SDK 子集已在新位置 `tools/bililive_dm_plugin_mock/sdk/` 单独维护。

当前拆分后，DanmuAI 侧入口是 `app/main_bililive_dm_mixin.py`、`app/main_request_context_mixin.py` 与 `app/main_render_coordinator_mixin.py`；协议与后台发送仍在 `app/application/bililive_dm_contracts.py` + `app/application/bililive_dm_push_service.py`。历史完成报告中的 `app/main_display_mixin.py` 已于后续架构拆分中删除。
...
| `repro_push_call.py` 成功但生成按钮无推送 | DanmuAI 版本过旧或未含 PUSH-004 挂钩；核对 `app/main_request_context_mixin.py` 与 `app/main_bililive_dm_mixin.py` |
| AI 文本能推送但公式化补位不显示 | 核对 `app/main_render_coordinator_mixin.py` 是否调用 `_schedule_bililive_dm_formula_push`，并确认来源是 `pool_topup` / `pool_duplicate_topup` / `meme_barrage` |
...
- 工单正文（FORMULA-008）：[.local-ai/workorders/active/W-BILILIVE-DM-PLUGIN-FORMULA-008.md](../../.local-ai/workorders/active/W-BILILIVE-DM-PLUGIN-FORMULA-008.md)
- 完成报告（FORMULA-008）：[reports/W-BILILIVE-DM-PLUGIN-FORMULA-008-completion-report.md](../../reports/W-BILILIVE-DM-PLUGIN-FORMULA-008-completion-report.md)
